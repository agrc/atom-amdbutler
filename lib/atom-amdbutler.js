...
module.exports =  {
    ...
    ensureModulesAreLoaded: function () {
        if (!crawler.modules) {
            ...
            //     return;
            // }
            var projectDirectory = atom.project.getDirectories()[0].path;
            var baseFolderPath = path.join(projectDirectory, 'src');

            if (!fs.existsSync(baseFolderPath)) {
                return;
            }

            var currentProject = path.join(baseFolderPath, 'app');
